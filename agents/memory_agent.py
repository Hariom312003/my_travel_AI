...
# Strict 7 Behavioral Preference Types
BEHAVIORAL_TYPES = {
    "pace_preference",
    "food_preference",
    "crowd_preference",
...
def extract_behavioral_memories(preferences: dict, feedback: str | None = None) -> list[dict]:
    ...
    if text_to_scan:
        ...
        # Comfort
        if any(w in text_to_scan for w in ["increase comfort", "luxury", "comfort stay", "better hotel", "premium lodging"]):
            memories.append({"type": "comfort_preference", "text": "prefers high comfort accommodation"})
...
def build_behavior_profile(
    ...
    if "high comfort" in lower or "luxury" in lower or "better hotel" in lower or "premium lodging" in lower:
