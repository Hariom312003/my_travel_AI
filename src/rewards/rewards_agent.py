...
def run_rewards_agent(structured_query: dict, budget_breakdown: dict) -> dict:
    ...
    logger.info("[Rewards Agent] Entering agent")
    logger.info(f"[Rewards Agent] Destination received: {dest}")
...
Provide specific reward optimization recommendations:"""
    ...
    res = rewards if isinstance(rewards, dict) and rewards.get("recommendations") else fallback
    logger.info(f"[Rewards Agent] Destination returned: {dest}")
    logger.info("[Rewards Agent] Leaving agent")
    return res
