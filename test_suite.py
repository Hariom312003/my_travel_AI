...
from agents.memory_agent import get_all_user_memory, _get_user_collection, retrieve_behavior_profile
...
class TestTravelPlanner(unittest.TestCase):
    ...
    def test_1_imports_and_compilation(self):
        ...
        modules = [
            ...
            "agents.query_agent", "agents.memory_agent", "agents.rag_agent", 
            "agents.planner_agent", "agents.budget_agent", "agents.rewards_agent", 
            "agents.validator_agent", "agents.refinement_agent", "agents.summary_agent",
            "graph.workflow", "api.app", "ui.streamlit_app"
        ]
        for mod in modules:
            try:
                __import__(mod)
