...
from agents.memory_agent import get_all_user_memory, retrieve_behavior_profile, store_behavioral_memory
...
def clear_user_memory(user_id: str):
    from agents.rag_agent import get_client
    try:
...
if menu == "🏠 Product Home & Sandbox":
    ...
    if plan_btn and user_query.strip():
        with st.spinner("🐝 Coordinating Agent Swarm..."):
            if api_online:
                try:
                    r = requests.post(f"{API_URL}/plan", json={"user_id": user_id, "query": user_query}, timeout=30)
                    if r.status_code == 200:
                        ...
                        st.error(f"Backend API Error: {r.json().get('detail')}")
                except Exception as e:
                ...
                initial_state: TravelState = {
                    ...
                    "changed_days": [], "stored_memories": [], "status": "started"
                }
                try:
                    ...
                    st.session_state.trip_state = {
                        ...
                        "changed_days": result.get("changed_days", []), "behavior_profile": profile, "status": "completed"
                    }
    ...
    if not state or not st.session_state.itinerary_generated:
        ...
        if menu == "🗺️ Live Travel Guide":
                    ...
                    if api_online:
                        ...
                        try:
                            ...
                            st.session_state.trip_state = {
                                ...
                                "budget": result.get("budget", {}), "rewards": result.get("rewards", {}),
                                "summary": result.get("summary", ""), "modification_intent": result.get("modification_intent", {}),
                                "changed_days": result.get("changed_days", []), "status": "completed"
                            }
                            st.session_state.refinement_history.append({
                    ...
                    if chain:
                        ...
                        for idx, step in enumerate(chain):
                            ...
                            <div style='display: flex; align-items: center; gap: 15px; position: relative;'>
                                <div style='background: #7b8cde; color: #0b0d19; font-weight: bold; width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 0 10px rgba(123, 140, 222, 0.5);'>
                                    {idx+1}
                                </div>
                                <div class='glass-card' style='margin: 0; padding: 12px 18px; flex-grow: 1; display: flex; justify-content: space-between; align-items: center;'>
                                    <div>
                                        <b style='color: #ffffff; font-size: 15px;'>📍 {step['location']}</b><br>
                                        <span style='color: #cbd5e1; font-size: 12px;'>⌚ Slot Time: {step['time']} &nbsp;•&nbsp; Duration: {step['duration']}</span>
                                    </div>
                                    {f"<div style='font-size: 12px; color: #7b8cde; background: rgba(123,140,222,0.1); padding: 4px 8px; border-radius: 6px;'>🚌 Transit: {step['transit']}</div>" if idx < len(chain)-1 else ""}
                                </div>
                            </div>
                            """
                            if idx < len(chain) - 1:
                                html_code += """
                                <div style='width: 32px; display: flex; justify-content: center; margin: -5px 0;'>
                                    <div style='border-left: 2px solid rgba(123, 140, 222, 0.4); height: 20px;'></div>
                                </div>
                                """
                        html_code += "</div>"
            ...
            # Agent Timings Grid
            st.markdown("#### Agent Execution Latencies")
